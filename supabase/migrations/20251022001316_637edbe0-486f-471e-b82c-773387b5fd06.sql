-- Corregir función notify_reporte_changes para usar profile ID en lugar de auth.uid()
-- El problema es que auth.uid() devuelve auth.users.id pero notifications.user_id
-- requiere profiles.id debido a la clave foránea
...
AS $function$
DECLARE
  action_user_profile_id UUID;
  reporte_nombre TEXT;
  assigned_user_name TEXT;
  creator_name TEXT;
BEGIN
  -- CORREGIDO: Usar get_profile_id_from_auth() en lugar de auth.uid()
  -- para obtener el profile.id en lugar de auth.users.id
  action_user_profile_id := get_profile_id_from_auth();
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
      ...
      -- CORREGIDO: Usar action_user_profile_id en lugar de action_user_id
      IF action_user_profile_id IS NOT NULL AND action_user_profile_id != NEW.user_id THEN
        PERFORM create_notification(
